...
from constants_vuer import grd_yup2grd_zup, hand2inspire, hand2Sim
...
class VuerPreprocessor:
    ...
    #     # change of basis
    #     head_mat = grd_yup2grd_zup @ self.vuer_head_mat @ fast_mat_inv(
    #         grd_yup2grd_zup)
    #     right_wrist_mat = grd_yup2grd_zup @ self.vuer_right_wrist_mat @ fast_mat_inv(
    #         grd_yup2grd_zup)
    #     left_wrist_mat = grd_yup2grd_zup @ self.vuer_left_wrist_mat @ fast_mat_inv(
    #         grd_yup2grd_zup)

    #     rel_left_wrist_mat = left_wrist_mat @ hand2inspire
    #     rel_left_wrist_mat[0:3, 3] = rel_left_wrist_mat[0:3,
    #                                                     3] - head_mat[0:3, 3]

    #     rel_right_wrist_mat = right_wrist_mat @ hand2inspire  # wTr = wTh @ hTr
    #     rel_right_wrist_mat[0:3,
    #                         3] = rel_right_wrist_mat[0:3, 3] - head_mat[0:3, 3]
    ...
    def process_v2(self, tv):
        #  Transform Head and Wrist Poses from VR to Sim World
        self.vuer_head_mat = mat_update(
            self.vuer_head_mat, tv.head_matrix.copy())
        self.vuer_right_wrist_mat = mat_update(
            self.vuer_right_wrist_mat, tv.right_hand.copy())
        ...
        # A change-of-basis matrix converting Y-up (common in VR) to Z-up (used in Sim environments like Isaac Sim).
        head_mat = grd_yup2grd_zup @ self.vuer_head_mat @ fast_mat_inv(
            grd_yup2grd_zup)
        right_wrist_mat = grd_yup2grd_zup @ self.vuer_right_wrist_mat @ fast_mat_inv(
            grd_yup2grd_zup)
        ...
        # Transform Wrist Poses into Sim-relative Hand Frames
        rel_left_wrist_mat = left_wrist_mat @ hand2Sim
        rel_right_wrist_mat = right_wrist_mat @ hand2Sim  # wTr = wTh @ hTr

        # Prepare Landmark (Finger Tip) Points as Homogeneous Coordinates
        left_fingers = np.concatenate(
            [tv.left_landmarks.copy().T, np.ones((1, tv.left_landmarks.shape[0]))])
        right_fingers = np.concatenate(
            [tv.right_landmarks.copy().T, np.ones((1, tv.right_landmarks.shape[0]))])

        # Transform Landmarks from VR Y-up to Sim Z-up World
        left_fingers = grd_yup2grd_zup @ left_fingers
        right_fingers = grd_yup2grd_zup @ right_fingers

        # Transform Landmarks into Wrist-relative Frame
        rel_left_fingers = fast_mat_inv(left_wrist_mat) @ left_fingers
        rel_right_fingers = fast_mat_inv(right_wrist_mat) @ right_fingers

        #  Final Basis Change into "Inspire" Hand Convention
        # (attention: hand2inspire should be renamed as inspire2hand)
        rel_left_fingers = (hand2inspire.T @ rel_left_fingers)[0:3, :].T
        rel_right_fingers = (hand2inspire.T @ rel_right_fingers)[0:3, :].T
